	...
	foreach my $post (@posts) {
		...
		if ($posts{$url}) {
			...
			if ($posts{$url} eq $content_hash) {
				...
				# post exists... but the content has changed
				print STDERR "c" if $DEBUG;
				my $update = $db->prepare("UPDATE posts SET content_hash=?, added_on=CURRENT_TIMESTAMP() WHERE url=?");
				$update->execute($content_hash, $url);

				my $activate = $db->prepare("UPDATE posts SET active=1 WHERE url=?");
				$activate->execute($url);
			}
		} else {
			# post doesn't exist in the database
			print STDERR "N" if $DEBUG;
			
			my $update = $db->prepare("INSERT INTO posts (blog_id, title, url, url_hash, content_hash, summary, filename, pubdate, added_on) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP())");
			$update->execute($blog_id, $details{"title"}, $url, md5_hex($url), $content_hash, $summary, $post, $details{"date"});
			...
			my $activate = $db->prepare("UPDATE posts SET active=1 WHERE post_id=?");
			$activate->execute($post_id);
