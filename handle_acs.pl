...
#        my $sql = $db->prepare("SELECT * FROM papers WHERE journal=? AND !ISNULL(doi_id)");
	$sql->execute($journal);
...
#		if ($page =~ /<FONT\sSIZE=+2><B>(.*?)<\/B><\/FONT>/) {
                if ($page =~ /<ATL><P><FONT\sSIZE=\+2><B>([^<]*)/) {
                        my $ptitle = $1;
                        $ptitle =~ s/\n|\r/\s/g;
                        $ptitle =~ s/\s(\s+)/\s/g;
                        $ptitle =~ s/^\s*//g;
                        $ptitle =~ s/\s*$//g;
			print STDERR "Got title: $ptitle\n";
